...
// Surface which branch/worktree a dev window is running so multiple
// `tauri:dev` instances (e.g. one per worktree) are distinguishable in
// the DEV/E2E pill (UpdaterBanner.tsx). Setting process.env here (rather
// than a `define`) lets Vite's normal VITE_-prefix pickup expose it as
// import.meta.env.VITE_GIT_BRANCH, same mechanism as VITE_MOCK_UPDATE.
