...
export function UpdaterBanner() {
  ...
  // dev-server startup — lets multiple `tauri:dev` windows (one per
  // worktree) be told apart at a glance. Absent for release builds.
  ...
  if (import.meta.env.DEV && !update && !hideDevPill) {
    return isE2E ? (
      ...
      >
        E2E
        {branch && (
          <span className="max-w-[140px] truncate font-mono text-[10px] font-normal normal-case tracking-normal opacity-80">
            {branch}
          </span>
        )}
      </span>
