...
fn pty_spawn(
    ...
    // One task lookup shared by the sandbox wrap and the remote
    // (ssh) rewrite below.
    let spawn_ws: Option<Task> = args
        .task_id
        .as_deref()
    ...
    // tasks never take this branch: effective_sandbox_mode() is
...
fn project_add_remote_sync(mut target: SshTarget, root_path: String) -> Result<Project, String> {
    ...
    let p = Project {
        ...
        name,
        root_path: expanded,
        // Local tasks_path is meaningless for a remote project; the
...
fn task_create_sync(args: CreateTaskArgs) -> Result<Task, String> {
    ...
    if let Some(target) = proj.ssh.clone() {
        return task_create_remote_sync(args, proj, target);
    }
    let repo = PathBuf::from(&proj.root_path);
...
/// Remote analog of `task_create_sync`: the worktree is created on
/// the project's ssh host, under `<remote_tasks_path>/<project>/
/// <slug>`. Same branch semantics (`--no-track` off the base, reuse an
/// existing branch), but all git / fs operations run over the
/// multiplexed connection and the resulting Task carries the
...
/// (remote tasks are always unsandboxed).
fn task_create_remote_sync(
    args: CreateTaskArgs,
    proj: Project,
    target: SshTarget,
) -> Result<Task, String> {
    ...
    if !pats.is_empty() {
        ...
        if let Err(e) = ssh_exec::run_remote(&target, &script, ssh_exec::SLOW, None) {
            rollback();
            return Err(format!("copying files into the task failed: {}", e));
        }
    }
    ...
    let ws = Task {
        ...
        has_resumable_history: false,
        agent_session_ids: std::collections::HashMap::new(),
        // The seatbelt sandbox is local-only; remote tasks run
        // unsandboxed on the host (surfaced in the UI).
        sandbox_enabled: false,
        sandbox_mode: Some(SandboxMode::Off),
        yolo: false,
    ...
    if let Err(e) = save_task(&ws) {
        rollback();
        return Err(e.to_string());
    }

    // Setup runs from the FRONTEND (launchSetupTab) through the same
    // host-aware task_run_script_stream path the local flow uses, so
    // nothing streams from here anymore (mirrors task_create_sync).
...
fn task_archive_sync(id: String, delete_branch: bool) -> Result<(), String> {
    ...
    if !w.composition.is_empty() {
        for m in w.composition.iter().rev() {
            ...
            // via `.termic.yaml` still tears down. Members are always local
            // (remote is single-repo only), so no ssh target here.
            let script = member_effective_script(m, |s| s.archive.clone(), &m.archive_script);
            if !script.trim().is_empty() && Path::new(&m.path).exists() {
                let _ = run_script(None, &script, &m.path, w.port, &w.name);
            }
        }
...
fn task_restore_sync(app: AppHandle, id: String) -> Result<Task, String> {
    ...
    if task.composition.is_empty() {
        let (setup, _, _) = effective_scripts(&proj);
        if !setup.trim().is_empty() {
            run_script_streaming(task.ssh.clone(), setup, wt_path.to_string_lossy().into_owned(), task.port, task.name.clone(), app, task.id.clone());
        }
    } else {
        for m in &task.composition {
...
#[tauri::command]
async fn task_diff(id: String) -> Result<String, String> {
    // async + spawn_blocking: three git invocations, each a network
    // round trip for remote tasks (docs/ipc.md discipline).
    tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
async fn task_changes(id: String) -> Result<TaskChanges, String> {
    ...
    // Member groups: only for multi-repo tasks (always local; a
    // remote task never has a composition). For each member, run
...
/// A String (not PathBuf) because for remote tasks this is a path
/// on the ssh host, never the local disk.
fn repo_cwd(w: &Task, dir_name: &str) -> Result<String, String> {
...
/// Git for a task-owned path: routed to the task's ssh host
...
fn wgit(w: &Task, args: &[&str], cwd: &str) -> Result<String> {
    ssh_exec::git_on(w.host(), args, cwd)
}

/// Remote analog of `safe_task_path`: lexically normalize a
/// renderer-supplied task-relative path. We cannot canonicalize a
/// remote path locally, so containment is enforced by rejecting
/// absolute paths and resolving `.`/`..` segments without ever letting
/// the result climb above the task root. (Symlink escape INSIDE
/// the task is accepted as a v1 non-goal — the same user already
...
/// Read a text file from a remote task, preserving the local
...
#[tauri::command]
async fn task_file_read(id: String, path: String) -> Result<String, String> {
    // async + spawn_blocking: a network round trip for remote tasks
    // (docs/ipc.md discipline; behavior-neutral locally).
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
...
/// path is constrained to the worktree by `safe_task_path` /
/// `safe_remote_rel`, same as the read side. Remote writes stage
/// through a temp file + `mv` so a dropped connection can't leave a
/// truncated file.
#[tauri::command]
async fn task_file_write(id: String, path: String, content: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
        let (cwd, rel) = resolve_task_git_path(&w, &path)?;
        if let Some(t) = &w.ssh {
...
#[tauri::command]
async fn task_path_rename(id: String, path: String, new_name: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
...
#[tauri::command]
async fn task_path_delete(id: String, path: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
        let (cwd, rel) = resolve_task_git_path(&w, &path)?;
        if let Some(t) = &w.ssh {
...
fn task_reveal_path(id: String, path: String) -> Result<(), String> {
    ...
    if w.ssh.is_some() {
        ...
        // frontend swaps this action for "Copy path" on remote tasks;
        // this guard covers any other caller.
        return Err("Reveal in Finder is not available for remote tasks.".into());
    }
...
/// directory (stat, reveal-in-file-manager) opt in instead. `cwd` is a
/// String because for remote tasks it is a path on the ssh host, not the
/// local disk.
fn resolve_task_git_path_ex(w: &Task, path: &str, allow_bare_member_root: bool) -> Result<(String, String), String> {
...
/// Resolve `(cwd, rel)` for a task file and read it, local or
/// remote, honoring the safe-path contract on both. `Ok(None)` = the
/// file doesn't exist (or isn't readable as UTF-8 text).
fn task_read_resolved(w: &Task, cwd: &str, rel: &str) -> Option<String> {
    match &w.ssh {
        Some(t) => {
        ...
        None => {
            let abs = safe_task_path(Path::new(cwd), rel).ok()?;
...
fn resolve_task_git_path(w: &Task, path: &str) -> Result<(String, String), String> {
...
fn task_file_diff_for_task(w: &Task, path: &str) -> Result<String, String> {
    ...
    if let Some(content) = task_read_resolved(w, &cwd, &rel_path) {
...
#[tauri::command]
async fn task_file_diff_sides(id: String, path: String) -> Result<FileDiffSides, String> {
    // async + spawn_blocking: a git round trip plus a file read, both
    // network hops for remote tasks (docs/ipc.md discipline).
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
...
#[tauri::command]
async fn task_file_diff(id: String, path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
...
#[tauri::command]
async fn task_files(id: String) -> Result<Vec<String>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let w = load_tasks().into_iter().find(|w| w.id == id).ok_or("no task")?;
        let mut out = Vec::new();
        if let Some(t) = &w.ssh {
...
fn task_dir_list_sync(id: String, rel: String, heal: bool) -> Result<Vec<FileEntry>, String> {
    ...
    // Remote tasks: one bounded round trip per directory. No
...
/// Build the Command that executes a user script for a task,
...
#[tauri::command]
async fn terminal_stage_file(task_id: String, src: String) -> Result<String, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // Remote task: the agent runs on the ssh host and can't read a
        // local temp path. Upload the dropped file into a staging dir under
        // the REMOTE task and hand back that path for the PTY paste.
        let ws = load_tasks().into_iter().find(|w| w.id == task_id);
...
#[tauri::command]
async fn run_capture_command(cmd: String, cwd: String, task_id: Option<String>) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let ssh = task_id
            .and_then(|wid| load_tasks().into_iter().find(|w| w.id == wid))
            .and_then(|w| w.ssh);
...
mod tests {
    ...
    fn safe_remote_rel_normalizes_and_contains() {
        ...
        assert_eq!(safe_remote_rel("a//b/").unwrap(), "a/b");
        // Interior .. resolves as long as it stays inside the task.
    ...
    fn task_path_stat_reports_a_bare_member_root_as_an_existing_directory() {
        ...
        let ws = task_with_member("frontend", host.path(), member.path());
        let (cwd, rel) = resolve_task_git_path_ex(&ws, "frontend", true).unwrap();
        let stat = check_task_path_existence(Path::new(&cwd), &rel).unwrap();
        assert!(stat.exists);
        assert!(stat.is_dir);
    }
