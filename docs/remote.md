...
workspaces path, and `root_path` is a path ON THAT HOST. Every workspace
created under a remote project lives on the host: the git worktree, the
agent process, shells, file ops, everything. The Workspace carries a
**frozen copy** of the target (`Workspace.ssh`, same freeze pattern as the
...
- `ssh_exec::git_on(host, args, cwd)` / `wgit(&workspace, args, cwd)` -
...
- `safe_remote_rel()` - lexical `.`/`..` normalization replacing
  `fs::canonicalize`-based `safe_workspace_path` for remote paths.
- PTY: `pty_spawn` rewrites a remote workspace's spawn into a LOCAL
